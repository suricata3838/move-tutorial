...
module NamedAddr::BasicCoin {
    ...
    public fun publish_balance(account: &signer) {
        ...
        // assert!(balance_of(signer::address_of(account)) == 0, EALREADY_HAS_BALANCE); //Null??(dagling of reference)
        // assert!(*&borrow_global<Balance>(signer::address_of(account)).coin == Coin{value:0}, EALREADY_HAS_BALANCE);
    ...
    fun deposit(_addr: address, check: Coin) acquires Balance {
        ...
        let bal_ref = &mut borrow_global_mut<Balance>(_addr).coin.value;
        // Increment the value by `amount`
        *bal_ref =  *bal_ref + _amount;
    }
}
